...
VALUE cCoBreakAttackWordlistStribog256;
VALUE cCoBreakAttackWordlistStribog512;
...
//Define Stribog-256 Crack
void calcular_stribog_256(const char *cadena, unsigned char *hash) {
    gcry_md_hd_t handle;
    gcry_md_open(&handle, GCRY_MD_STRIBOG256, 0);
    gcry_md_write(handle, cadena, strlen(cadena));
    gcry_md_final(handle);
    memcpy(hash, gcry_md_read(handle, GCRY_MD_STRIBOG256), 32);
    gcry_md_close(handle);
}

int comparar_hashes_stribog_256(const unsigned char *hash1, const unsigned char *hash2) {
    return memcmp(hash1, hash2, 32) == 0;
}

void hex_a_hash_stribog_256(const char *hex, unsigned char *hash) {
    for (size_t i = 0; i < 32; i++) {
        sscanf(hex + 2 * i, "%2hhx", &hash[i]);
    }
}

VALUE attackwordlist_stribog_256(VALUE self, VALUE hash, VALUE dictionary) {
    FILE *archivo = fopen(StringValueCStr(dictionary), "r");
    if (archivo == NULL) {
        rb_raise(rb_eIOError, "Error al abrir el archivo de texto");
    }
    ...
    unsigned char hash_objetivo[32];
    hex_a_hash_stribog_256(StringValueCStr(hash), hash_objetivo);
    ...
    while (1) {
        ...
        for (size_t i = 0; i < count; i++) {
            calcular_stribog_256(lineas[i], hash_actual);
 
            if (comparar_hashes_stribog_256(hash_actual, hash_objetivo)) {
                if (found_password == Qnil) {
                    found_password = rb_str_new_cstr(lineas[i]);
                }
            }
...
//Define Stribog-512 Crack
void calcular_stribog_512(const char *cadena, unsigned char *hash) {
    gcry_md_hd_t handle;
    gcry_md_open(&handle, GCRY_MD_STRIBOG512, 0);
    gcry_md_write(handle, cadena, strlen(cadena));
    gcry_md_final(handle);
    memcpy(hash, gcry_md_read(handle, GCRY_MD_STRIBOG512), 64);
    gcry_md_close(handle);
}

int comparar_hashes_stribog_512(const unsigned char *hash1, const unsigned char *hash2) {
    return memcmp(hash1, hash2, 64) == 0;
}

void hex_a_hash_stribog_512(const char *hex, unsigned char *hash) {
    for (size_t i = 0; i < 64; i++) {
        sscanf(hex + 2 * i, "%2hhx", &hash[i]);
    }
}

VALUE attackwordlist_stribog_512(VALUE self, VALUE hash, VALUE dictionary) {
    FILE *archivo = fopen(StringValueCStr(dictionary), "r");
    if (archivo == NULL) {
        rb_raise(rb_eIOError, "Error al abrir el archivo de texto");
    }
    ...
    unsigned char hash_objetivo[64];
    hex_a_hash_stribog_512(StringValueCStr(hash), hash_objetivo);
    ...
    while (1) {
        ...
        for (size_t i = 0; i < count; i++) {
            calcular_stribog_512(lineas[i], hash_actual);
 
            if (comparar_hashes_stribog_512(hash_actual, hash_objetivo)) {
                if (found_password == Qnil) {
                    found_password = rb_str_new_cstr(lineas[i]);
                }
            }
