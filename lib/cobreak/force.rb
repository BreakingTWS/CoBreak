...
class Forze_brute
  ...
  def word(dato, wordlist, type, out, verbose = false)
    ...
    elsif (type_hash.downcase.eql?('blake2b-160'))
      forzebrute.time = Time.now
      @result = CoBreak::AttackWordlist::BLAKE2B_160.crack(hash_input, wordlist)
    elsif (type_hash.downcase.eql?('blake2s-224'))
      forzebrute.time = Time.now
      @result = CoBreak::AttackWordlist::BLAKE2S_224.crack(hash_input, wordlist)
    ...
    elsif (type_hash.downcase.eql?('blake2b-384'))
      forzebrute.time = Time.now
      @result = CoBreak::AttackWordlist::BLAKE2B_384.crack(hash_input, wordlist)
