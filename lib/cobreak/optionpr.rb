  ...
  class ParseOPT
    def self.optparse(options)
      ...
      OptionParser.new do|param|
        ...
        param.on('-bword', '--brutehash=DIGEST', String, 'brute force mode to crack a hash whitch wordlist format'){|modeforcehash|options.bruteforce = modeforce}
        param.on('-bchar', '--brutehashchar=DIGEST', String, 'brute force mode to crack a hash whitch char generator format'){|modeforcehashchar|options.bruteforce = modeforce}
        param.on('-bcipherw', '--brutecipher=CIPHER', String, 'brute force mode to crack a hash cipher wordlist format'){|modeforcechiper|options.bruteforce = modeforce}
        param.on('-bcipherw', '--brutecipherchar=CIPHER', String, 'brute force mode to crack a cipher whitch char generator'){|modeforcecipherchar|options.bruteforce = modeforce}
        param.separator ""
